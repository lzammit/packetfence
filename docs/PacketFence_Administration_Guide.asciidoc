...
We added the ability to specify filters directly in the portion of code that revaluate the vlan. 

These rules are available in different scope: 
...
And can be defined based on differents criteria like:
...
Per example let's define a rule that doesn't allow the connection to a device when his category is default and the ssid is SECURE and the current date is between 11am and 2pm
from Monday to Friday when it try to connect as a registered device :
  ...
  filter = time
  operator = is
  regexp = wd {Mon Tue Wed Thu Fri} hr {11am-2pm}

  [1:1&2&3]
  scope = NormalVlan
  action = nointernet

You can have a look in the file vlan_filters.conf, there are some example how to use and define filters.
